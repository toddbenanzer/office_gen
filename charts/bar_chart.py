"""
Bar chart implementation for the pptx_charts_tables package.
"""
...
class BarChart(Chart):
    ...
    def _create_chart(self):
        ...
        # Default to column_clustered (vertical bar chart)
        chart_type = self.kwargs.get(
            "chart_type", self.config["bar_chart"].get("chart_type", "column_clustered")
        )
    ...
    def _apply_styles(self):
        ...
        if hasattr(self.chart.plots[0], "gap_width"):
            gap_width = self.kwargs.get(
                "gap_width", self.config["bar_chart"].get("gap_width", 100)
            )
