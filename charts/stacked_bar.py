"""
Stacked bar chart implementation for the pptx_charts_tables package.
"""
...
class StackedBarChart(Chart):
    ...
    def _create_chart(self):
        ...
        # Determine chart type (vertical or horizontal, standard or 100%)
        chart_type = self.kwargs.get(
            "chart_type",
            self.config["stacked_bar_chart"].get("chart_type", "column_stacked"),
        )

        chart_types = {
            "column_stacked": XL_CHART_TYPE.COLUMN_STACKED,  # Vertical bars
            "column_stacked_100": XL_CHART_TYPE.COLUMN_STACKED_100,  # Vertical 100%
            "bar_stacked": XL_CHART_TYPE.BAR_STACKED,  # Horizontal bars
            "bar_stacked_100": XL_CHART_TYPE.BAR_STACKED_100,  # Horizontal 100%
        }

        xl_chart_type = chart_types.get(chart_type, XL_CHART_TYPE.COLUMN_STACKED)
    ...
    def _apply_styles(self):
        """Apply styles to the stacked bar chart."""
        # Set chart title
        ...
        if hasattr(self.chart.plots[0], "gap_width"):
            gap_width = self.kwargs.get(
                "gap_width", self.config["stacked_bar_chart"].get("gap_width", 100)
            )
        ...
        # Set data label position (specific to stacked bar)
        self._set_data_label_position()

        # Set series colors
        self._set_series_colors()

    def _set_data_label_position(self):
        """Set data label position, often 'inside_end' for stacked bars."""
        ...
        position = self.kwargs.get(
            "data_label_position",
            self.config["stacked_bar_chart"].get("data_label_position", "inside_end"),
        )
        ...
        if position in positions:
            plot.data_labels.position = positions[position]

    def _set_series_colors(self):
        """Set colors for chart series."""
        # Get data label font color - for stacked bars, often white
        data_label_font_color = self.kwargs.get(
            "data_label_font_color",
            self.config["stacked_bar_chart"].get("data_label_font_color", "FFFFFF"),
        )

        # Set global data label text color if specified
        if hasattr(self.chart.plots[0], "data_labels") and data_label_font_color:
            self.chart.plots[0].data_labels.font.color.rgb = RGBColor.from_string(
                data_label_font_color
            )
        ...
        for i, series in enumerate(self.chart.series):
            ...
            if color:
                ...
                # Per-series data label settings
                if hasattr(series, "data_labels"):
                    # Show value for each data point
